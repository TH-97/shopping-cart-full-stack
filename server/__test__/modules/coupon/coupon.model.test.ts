...
describe('Coupon.isApplicable', () => {
  ...
  test('사용 시간대 밖이면 false', () => {
    const coupon = createCoupon({ usableFrom: '13:00', usableTo: '14:00' });
    // now 10:00 (UTC, getHours는 로컬이지만 동일 비교 목적상 경계만 본다)
    const at1200 = new Date('2026-06-20T12:00:00');
    expect(coupon.isApplicable(ctx({ now: at1200 }))).toBe(false);
  });

  test('사용 시간대 안이면 true(경계 포함)', () => {
    const coupon = createCoupon({ usableFrom: '13:00', usableTo: '14:00' });
    const at1300 = new Date('2026-06-20T13:00:00');
    expect(coupon.isApplicable(ctx({ now: at1300 }))).toBe(true);
  });
